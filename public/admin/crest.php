...
if ($action === 'delete') {
    ...
    $upd = $pdo->prepare("UPDATE guilds SET crest_file = NULL, updated_at = datetime('now') WHERE id = :id");
...
if (!isset($_FILES['crest']) || !is_uploaded_file($_FILES['crest']['tmp_name'])) {
...
if (class_exists('Imagick')) {
    ...
    if (!function_exists('imagewebp')) {
        flash('err', 'GD hat kein WebP-Support. Installiere php8.3-gd mit WebP oder nutze Imagick.');
        redirect($returnTo);
    }
...
// DB updaten
$upd = $pdo->prepare("UPDATE guilds SET crest_file = :f, updated_at = datetime('now') WHERE id = :id");
