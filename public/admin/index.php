	...
	if ($_SERVER["REQUEST_METHOD"] === "POST") {
		...
		try {
            ...
            ($action === "create_user" && !$usersExist)
				...
				if ($pass2 === "") {
					$pass2 = $pass1;
				} // erlaubt 1-Passwort-Feld (ältere Form)
			...
			if ($action === "add_guild" || $action === "create_guild") {
				...
				$stmt = $pdo->prepare(
                'INSERT INTO guilds (name, server, tag, notes, updated_at) VALUES (:n, :s, :t, :no, datetime(\'now\'))',
				);
				$stmt->execute([
                ":n" => $name,
                ":s" => $server,
			...
			if ($action === "delete_guild") {
				...
				$stmt = $pdo->prepare(
                "SELECT crest_file FROM guilds WHERE id = :id",
				);
				$stmt->execute([":id" => $id]);
				...
				if ($crestFile !== "") {
					...
					if ($baseDir !== false) {
						$target =
                        $baseDir . DIRECTORY_SEPARATOR . basename($crestFile);
						if (is_file($target)) {
							@unlink($target);
						}
					}
