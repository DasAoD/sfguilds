...
/* -------------------------
   Guild list
-------------------------- */
$guilds = $pdo->query("SELECT id, server, name, tag FROM guilds ORDER BY server, name")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'clear_missing') {
    unset($_SESSION['_missing_members'][$guildId], $_SESSION['_missing_members_ts'][$guildId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'import_csv') {
    ...
    foreach ($header as $i => $col) {
        ...
        elseif ($k === 'zulonline' || $k === 'zulonline') $map['last_online'] = $i; // (nur zur Sicherheit)
        elseif ($k === 'zulonline' || $k === 'zulonline') $map['last_online'] = $i;
        elseif ($k === 'zulonline' || $k === 'zulonline') $map['last_online'] = $i;
        elseif ($k === 'zulonline' || $k === 'zulonline') $map['last_online'] = $i;
    ...
    $sel = $pdo->prepare("SELECT id, fired_at, left_at, notes, rank FROM members WHERE guild_id = ? AND name = ? LIMIT 1");
    ...
    try {
        while (($row = fgetcsv($fh, 0, $delimiter)) !== false) {
            ...
            if (isset($seen[$nameKey])) { $skipped++; continue; }
            $seen[$nameKey] = true;
            ...
            $mentorStr      = trim(toUtf8((string)csvGet($row, $map, 'mentor', '')));
            $knightHallStr  = trim(toUtf8((string)csvGet($row, $map, 'knight_hall', '')));
            $guildPetStr    = trim(toUtf8((string)csvGet($row, $map, 'guild_pet', '')));
            $daysOfflineStr = trim(toUtf8((string)csvGet($row, $map, 'days_offline', '')));
            ...
            if ($firedRaw !== '') {
                ...
                if ($fired === null) { $errors[] = "Ungültiges Datum Entlassen bei {$name}: {$firedRaw}"; continue; }
            }
            if ($leftRaw !== '') {
                $left = normalizeDateDE($leftRaw);
                if ($left === null) { $errors[] = "Ungültiges Datum Verlassen bei {$name}: {$leftRaw}"; continue; }
            }
            ...
            if ($existing) {
                ...
                if ($firedRaw !== '' && $newFired) $newLeft = null;
                if ($leftRaw  !== '' && $newLeft)  $newFired = null;
...
$pdo->prepare("UPDATE guilds SET last_import_at = :ts, updated_at = datetime('now') WHERE id = :id")
    ->execute([
        ':ts' => gmdate('c'),
        ':id' => $guildId,
    ]);
    ...
    foreach ($activeRows as $r) {
        ...
        if (!isset($seen[$k])) {
            $missing[] = ['id' => (int)$r['id'], 'name' => (string)$r['name']];
        }
    }
    ...
    if (!empty($errors)) {
        ...
        foreach ($errors as $err) error_log("CSV-Import: " . $err);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'save_member') {
    ...
    if ($guildIdPost > 0) $guildId = $guildIdPost;
...
view('layout', [
    ...
    'content' => function() use ($guilds, $guildId, $guild, $members, $total, $active, $firedCount, $leftCount, $missing, $missingTs) { ?>
            ...
            <label>Gilde:
                <select name="guild_id">
                    <?php foreach ($guilds as $g): ?>
                        <option value="<?= (int)$g['id'] ?>" <?= ((int)$g['id'] === (int)$guildId) ? 'selected' : '' ?>>
                            <?= h($g['server']) ?> – <?= h($g['name']) ?><?= !empty($g['tag']) ? ' [' . h($g['tag']) . ']' : '' ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </label>
...
<?php endif; ?>
        ...
        <?php if (!empty($missing)): ?>
            <div class="box" style="margin: 1rem 0;">
                <h3 style="margin-top:0;">Fehlende Mitglieder im aktuellen Export (<?= count($missing) ?>)</h3>
                <?php if ($missingTs): ?>
                    <p class="muted" style="margin-top:0;">Stand: <?= h($missingTs) ?> UTC</p>
                <?php endif; ?>
                <p class="muted">
                ...
                <ul class="list">
                    <?php foreach ($missing as $x): ?>
                        <li><a href="#m<?= (int)$x['id'] ?>"><?= h($x['name']) ?></a></li>
                    <?php endforeach; ?>
                </ul>
          ...
          <table id="members" class="table members-table">
            ...
            <?php foreach ($members as $i => $m): ?>
                ...
                <tr id="m<?= $mid ?>" class="<?= h(memberRowClass($m)) ?>">
                  ...
                  <td><?= h($daysOffline === null ? '' : (string)$daysOffline) ?></td>
                  <td><input form="<?= $formId ?>" name="fired_at" value="<?= h(formatDateDE($m['fired_at'] ?? '')) ?>" placeholder="TT.MM.JJJJ"></td>
				  <td><input form="<?= $formId ?>" name="left_at" value="<?= h(formatDateDE($m['left_at'] ?? '')) ?>" placeholder="TT.MM.JJJJ"></td>
                  <td><textarea form="<?= $formId ?>" name="notes" rows="2" style="width:100%;"><?= h($m['notes'] ?? '') ?></textarea></td>
...
if (!function_exists('normalizeNumber')) {
    function normalizeNumber($v): ?int { return normalize_number($v); }
}

if (!function_exists('normalizeDate')) {
    function normalizeDate(?string $v): ?string { return normalize_date($v); }
}

if (!function_exists('rowClass')) {
    function rowClass(array $m): string { return row_class($m); }
}

if (!function_exists('csrfToken')) {
    function csrfToken(): string { return csrf_token(); }
}

if (!function_exists('csrfCheck')) {
    function csrfCheck(): void { csrf_check(); }
}
