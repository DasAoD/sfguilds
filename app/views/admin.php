...
<?php if (empty($usersExist)): ?>
  <div class="card" style="padding:12px;">
    ...
    <?php else: ?>
      ...
      <pre class="box" style="white-space:pre-wrap;">touch <?= e((string)($setupFlag ?? 'storage/allow_setup')) ?></pre>
...
<?php else: ?>
      ...
      <?php
        ...
        if (!empty($editId)) {
          foreach (($guilds ?? []) as $gg) {
            if ((int)($gg['id'] ?? 0) === (int)$editId) { $editGuild = $gg; break; }
          }
        }
      ?>
          ...
          <form method="post" action="<?= e(url('/admin/')) ?>" class="guild-form">
            ...
            <div class="grid">
              <div class="field">
                ...
                <input type="text" name="name" required value="<?= e((string)$editGuild['name']) ?>">
              </div>

              <div class="field">
                <label>Server</label>
                <input type="text" name="server" required value="<?= e((string)$editGuild['server']) ?>">
              </div>

              <div class="field">
                <label>Tag</label>
                <input type="text" name="tag" placeholder="Optional" value="<?= e((string)($editGuild['tag'] ?? '')) ?>">
              </div>

              <div class="field">
                <label>Notiz</label>
                <textarea name="notes" rows="1" placeholder="Optionale Beschreibung"><?= e((string)($editGuild['notes'] ?? '')) ?></textarea>
              </div>
            </div>
              ...
              <tbody>
                <?php foreach ($guilds as $g): ?>
                  <tr>
                    ...
                    <td class="actions">
                      <a class="btn small" href="<?= e(url('/admin/?edit=' . (int)$g['id'])) ?>">Bearbeiten</a>
                      <a class="btn small" href="<?= e(url('/admin/members.php?guild_id=' . (int)$g['id'])) ?>">Mitglieder</a>
  ...
  <form method="post" action="<?= e(url('/admin/')) ?>" style="display:inline-block; margin-left:10px;">
