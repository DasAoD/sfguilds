...
function usage(): void {
    fwrite(STDERR, "Usage:\n");
    fwrite(STDERR, "  php cli/import_sftools.php --file /path/to/guild.csv [--guild-id 1]\n");
...
function argValue(array $argv, string $name): ?string {
    $n = count($argv);
    for ($i = 0; $i < $n; $i++) {
        if ($argv[$i] === $name && isset($argv[$i+1])) return (string)$argv[$i+1];
    }
    return null;
}
...
function guildIdFromFile(PDO $pdo, string $filePath): int {
    ...
    foreach ($rows as $g) {
        ...
        if ($nameSlug === $key) return (int)$g['id'];

        $tag = (string)($g['tag'] ?? '');
        if ($tag !== '' && slugify($tag) === $key) return (int)$g['id'];
    }
...
function importMembersCsv(PDO $pdo, int $guildId, string $csvPath): array
{
    ...
    if (!$header || count($header) < 2) {
        fclose($fh);
        throw new RuntimeException("CSV-Header konnte nicht gelesen werden: {$csvPath}");
    }
    ...
    try {
        while (($row = fgetcsv($fh, 0, $delimiter)) !== false) {
            ...
            if ($existing) {
                ...
                if ($firedRaw !== '' && !empty($newFired)) $newLeft = null;
...
if (hasFlag($argv, '--list-guilds')) {
    $pdo = db();
    $rows = $pdo->query("SELECT id, server, name, tag FROM guilds ORDER BY server, name")->fetchAll();
    foreach ($rows as $g) {
